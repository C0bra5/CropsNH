...
public class MTECropBreeder extends MTEBasicMachine {
    ...
    private static int getInputSlotCount(int tier) { return tier < VoltageIndex.HV ? MIN_INPUT_SLOT_COUNT : MAX_INPUT_SLOT_COUNT; }
    ...
    public int checkRecipe(boolean skipOC) {
        ...
        deterministicMutations.sort(Comparator.comparing(ICropMutation::getParentCount).reversed());
        ...
        for (ICropMutation mutation : deterministicMutations) {
            ...
            int amountOfFluidToConsume = drainPerTier * mutation.getOutput().getTier();
            if (amountOfFluidToConsume > this.mFluid.amount) continue;

            // abort early if the output doesn't fit.
            ItemStack newSeed = mutation.getOutput().getSeedItem(getNewSeedStats(mutation, breedingParents));
            newSeed.stackSize = 1;
            ...
            if (!skipOC) {
                this.calculateOverclockedNess(mutation.getBreedingMachineRecipeEUt(), mutation.getBreedingMachineRecipeDuration());
    ...
    private static ISeedStats getNewSeedStats(ICropMutation mutation, HashMap<ICropCard, SeedData> breedingParents) {
        ...
        for (ICropCard parent : mutation.getParents()) {
            ISeedStats parentStats = breedingParents.get(parent).getStats();
