...
public class CropBreederRecipeLoader extends BaseGTRecipeLoader {
    ...
    public static void postInit() {
        for (ICropMutation mutation : MutationRegistry.instance.getDeterministicMutations()) {
            List<Object> inputs = mutation.getParents().stream().map(parent -> {
            ...
            int tier = mutation.getOutput().getTier();
            ...
            GTRecipeBuilder template = GTValues.RA.stdBuilder()
                ...
                .itemOutputs(mutation.getOutput().getSeedItem(SeedStats.DEFAULT_ANALYZED))
                .nbtSensitive();
            ...
            for (Map.Entry<Fluid, Integer> allowedFluid : MTECropBreeder.ALLOWED_LIQUID_FERTILIZER.entrySet()) {
                GTRecipeBuilder builder = template
                    .copy()
                    .fluidInputs(new FluidStack(allowedFluid.getKey(), mutation.getOutput().getTier()*allowedFluid.getValue()));

                if (catalysts == null || catalysts.isEmpty()) {
                    builder.addTo(CropsNHGTRecipeMaps.fakeCropBreederRecipeMap);
                }
                else {
                    Optional<GTRecipe.GTRecipe_WithAlt> opt = builder.forceOreDictInput().buildWithAlt();
                    if (opt.isPresent()) {
